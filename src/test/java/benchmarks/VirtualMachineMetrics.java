...
public class VirtualMachineMetrics {
	...
	public static Map<String, Number> fetch() {
		try {
			MBeanServer connection = ManagementFactory.getPlatformMBeanServer();
			Map<String, Number> metrics = new HashMap<>(new BufferPools(connection).getMetrics());
	...
	static void gc(MBeanServer mBeanServer) {
		try {
...
class BufferPools {
	...
	private static final String[] ATTRIBUTES = { "Code Cache", "Compressed Class Space", "Metaspace", "PS Eden Space",
			"PS Old Gen", "PS Survivor Space", "G1 Eden Space", "G1 Old Gen", "G1 Survivor Space",
